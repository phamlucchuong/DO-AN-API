...
async function postCv(uid, cvData, file) {
  try {
    const formData = new FormData();
    formData.append("data", file); // file pdf
    formData.append("id", uid); // job ID
    formData.append("uid_employee", cvData.employee_id); // user ID

    const response = await fetch('https://c6fd-2405-4803-c786-ef80-e87b-3be8-3d79-c151.ngrok-free.app/webhook-test/upload_file', {
      method: 'POST',
      body: formData,
    });

    if (!response.ok) {
      console.error('Lỗi khi gửi CV đến webhook');
    } else {
      console.log('CV đã được gửi thành công đến webhook!');
    }
  } catch (error) {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (job) {
    ...
    document.getElementById("companyName").textContent = job.employer.companyName;
    document.getElementById("jobSalary").textContent = `${job.salary} VNĐ`;
    document.getElementById("jobLocation").innerHTML = `<i class="fas fa-map-marker-alt"></i> ${job.address}`;
    document.getElementById("jobExperience").innerHTML = `<i class="fas fa-briefcase"></i> ${job.experience}`;
    document.getElementById("jobDescription").textContent = job.description;
    document.getElementById("postedDate").textContent = `Đăng ngày: ${formatDate(job.createdAt)}`;

    if (job.isHot) document.getElementById("jobHotBadge").style.display = "inline-block";
    if (job.isUrgent) document.getElementById("jobUrgentBadge").style.display = "inline-block";

    document.getElementById("jobSkills").textContent = job.requirements;
    ...
    document.getElementById("jobDetail").innerHTML = '<div class="container"><h2>Không tìm thấy công việc</h2></div>';
  }
