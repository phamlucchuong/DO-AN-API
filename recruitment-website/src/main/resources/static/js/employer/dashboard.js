...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (
    ...
    alert(
      "Lỗi giao diện: Một số phần tử không được tìm thấy. Vui lòng liên hệ hỗ trợ."
    );
    return;
  }
  ...
  function loadTotalCandidates() {
    fetch(`/api/application/employer/${payload.user_id}/count`, {
      method: "GET",
      headers: {
      ...
      .then((response) => {
        if (!response.ok) {
          return response.json().then((err) => {
            throw new Error(err.message || "Lỗi khi lấy tổng ứng viên");
          });
        }
      ...
      .catch((error) => {
        ...
        totalCandidatesElement.textContent = "0";
      });
  }
    ...
    fetch(
      ...
      .then((response) => {
        if (!response.ok) {
          return response.json().then((err) => {
            throw new Error(
              err.message || "Lỗi khi tải dữ liệu tin tuyển dụng"
            );
          });
        }
        return response.json();
      ...
      .then((data) => {
        ...
        const totalCandidates = data.jobs
          ? data.jobs.reduce((sum, job) => sum + (job.applicationCount || 0), 0)
          : 0;
        ...
        data.jobs.forEach((job) => {
          const urgentTag =
            normalizeStatus(job.status) === "urgent"
            ...
            <div class="job-card" id="job-${job.id}">
              ...
              <div class="job-actions">
                <button class="btn btn-secondary" data-job-id="${
                  job.id
                }" data-action="detail">👥 Xem chi tiết</button>
                <button class="btn" data-job-id="${
                  job.id
                }" data-action="edit">✏️ Chỉnh sửa</button>
              </div>
              <div class="job-detail" id="job-detail-${
                job.id
              }" style="display: none; margin-top: 10px; padding: 10px; background: #f9f9f9; border: 1px solid #ccc;">
                <p>Đang tải chi tiết...</p>
              </div>
              <div class="job-edit" id="job-edit-${
                job.id
              }" style="display: none; margin-top: 10px; padding: 10px; background: #f9f9f9; border: 1px solid #ccc;">
      ...
      fetch(`/api/employer/job/${jobId}/detail`, {
        ...
        .then((job) => {
          detailDiv.innerHTML = `
            ...
            <p><strong>Yêu cầu:</strong> ${formatMultilineText(
              job.requirements
            )}</p>
            <p><strong>Mô tả:</strong> ${formatMultilineText(
              job.description
            )}</p>
            <p><strong>Phúc lợi:</strong> ${formatMultilineText(
              job.benefits
            )}</p>
            <p><strong>Thời gian làm việc:</strong> ${getWorkingHoursDisplayName(
              job.workingHours
            )}</p>
            ...
            <p><strong>Trạng thái:</strong> ${getStatusDisplayName(
              job.status
            )}</p>
            <p><strong>Đã duyệt:</strong> ${
              job.isApproved ? "Đã duyệt" : "Chưa duyệt"
            }</p>
            <button data-job-id="${
              job.id
            }" data-action="close-detail" style="margin-top: 10px;">Đóng</button>
          `;
          ...
          editDiv.innerHTML = `
            ...
            <form id="edit-job-form-${job.id}">
              <div class="form-group">
                <label for="job-title-${
                  job.id
                }"><strong>Tiêu đề:</strong></label>
                <input type="text" id="job-title-${
                  job.id
                }" name="title" value="${
            job.title || ""
          }" required style="width: 100%; margin-bottom: 10px;" placeholder="e.g. Senior Software Engineer">
              </div>
              <div class="form-group">
                <label for="companyName-${
                  job.id
                }"><strong>Công ty:</strong></label>
            ...
            job.address || ""
          }" style="width: 100%; margin-bottom: 10px;" placeholder="e.g. 123 Lê Duẩn">
              </div>
              <div class="form-group">
                <label for="city-${job.id}"><strong>Thành phố:</strong></label>
                <select id="city-${
                  job.id
                }" name="city" style="width: 100%; margin-bottom: 10px;">
                  <option value="${job.city}" disabled ${
            !job.city ? "selected" : ""
          }>Chọn thành phố</option>
                  ${cities
                    .map(
                      (city) => `
                    <option value="${city}" ${
                        job.city === city ? "selected" : ""
                      }>${city}</option>
                  `
                    )
                    .join("")}
              ...
              <div class="form-group">
                <label for="employmentType-${
                  job.id
                }"><strong>Loại hình công việc:</strong></label>
                <select id="employmentType-${
                  job.id
                }" name="employmentType" style="width: 100%; margin-bottom: 10px;" required>
                  <option value="" disabled ${
                    !job.employmentType ? "selected" : ""
                  }>Chọn loại hình công việc</option>
                  <option value="FULL_TIME" ${
                    job.employmentType === "FULL_TIME" ? "selected" : ""
                  }>Toàn thời gian</option>
                  <option value="PART_TIME" ${
                    job.employmentType === "PART_TIME" ? "selected" : ""
                  }>Bán thời gian</option>
                  <option value="CONTRACT" ${
                    job.employmentType === "CONTRACT" ? "selected" : ""
                  }>Hợp đồng</option>
                  <option value="INTERNSHIP" ${
                    job.employmentType === "INTERNSHIP" ? "selected" : ""
                  }>Thực tập</option>
                  <option value="FREELANCE" ${
                    job.employmentType === "FREELANCE" ? "selected" : ""
                  }>Tự do</option>
                </select>
              </div>
              <div class="form-group">
                ...
                <select id="jobLevel-${
                  job.id
                }" name="jobLevel" style="width: 100%; margin-bottom: 10px;" required>
                  <option value="" disabled ${
                    !job.jobLevel ? "selected" : ""
                  }>Chọn cấp độ</option>
                  <option value="INTERN" ${
                    job.jobLevel === "INTERN" ? "selected" : ""
                  }>Thực tập sinh</option>
                  <option value="FRESH_GRADUATE" ${
                    job.jobLevel === "FRESH_GRADUATE" ? "selected" : ""
                  }>Mới ra trường</option>
                  <option value="STAFF" ${
                    job.jobLevel === "STAFF" ? "selected" : ""
                  }>Nhân viên</option>
                  <option value="MANAGER" ${
                    job.jobLevel === "MANAGER" ? "selected" : ""
                  }>Trưởng phòng</option>
                  <option value="DIRECTOR_OR_HIGHER" ${
                    job.jobLevel === "DIRECTOR_OR_HIGHER" ? "selected" : ""
                  }>Giám Đốc</option>
                </select>
              </div>
              <div class="form-group">
                <label for="salary-${
                  job.id
                }"><strong>Mức lương:</strong></label>
                <input type="text" id="salary-${job.id}" name="salary" value="${
            job.salary || ""
          }" style="width: 100%; margin-bottom: 10px;" placeholder="e.g. $80,000 - $120,000">
              </div>
              <div class="form-group">
                <label for="experience-${
                  job.id
                }"><strong>Kinh nghiệm:</strong></label>
                <input type="text" id="experience-${
                  job.id
                }" name="experience" value="${
            job.experience || ""
          }" style="width: 100%; margin-bottom: 10px;" placeholder="e.g. 3+ years">
              </div>
              <div class="form-group">
                <label for="requirements-${
                  job.id
                }"><strong>Yêu cầu:</strong></label>
                <textarea id="requirements-${
                  job.id
                }" name="requirements" style="width: 100%; height: 100px; margin-bottom: 10px;">${
            job.requirements || ""
          }</textarea>
              </div>
              <div class="form-group">
                ...
                <textarea id="description-${
                  job.id
                }" name="description" style="width: 100%; height: 100px; margin-bottom: 10px;">${
            job.description || ""
          }</textarea>
              </div>
              <div class="form-group">
                <label for="benefits-${
                  job.id
                }"><strong>Phúc lợi:</strong></label>
                <textarea id="benefits-${
                  job.id
                }" name="benefits" style="width: 100%; height: 100px; margin-bottom: 10px;">${
            job.benefits || ""
          }</textarea>
              </div>
              <div class="form-group">
                <label for="workingHours-${
                  job.id
                }"><strong>Thời gian làm việc:</strong></label>
                <select id="workingHours-${
                  job.id
                }" name="workingHours" style="width: 100%; margin-bottom: 10px;" required>
                  <option value="" disabled ${
                    !job.workingHours ? "selected" : ""
                  }>Chọn thời gian làm việc</option>
                  <option value="EIGHTTOSEVENTEEN" ${
                    job.workingHours === "EIGHTTOSEVENTEEN" ? "selected" : ""
                  }>Từ 8AM đến 17PM</option>
                  <option value="NINETOEIGHTEEN" ${
                    job.workingHours === "NINETOEIGHTEEN" ? "selected" : ""
                  }>Từ 9AM đến 18PM</option>
                </select>
              </div>
              <div class="form-group">
                <label for="numberOfVacancies-${
                  job.id
                }"><strong>Số lượng tuyển:</strong></label>
                <input type="number" id="numberOfVacancies-${
                  job.id
                }" name="numberOfVacancies" value="${
            job.numberOfVacancies || 1
          }" min="1" style="width: 100%; margin-bottom: 10px;">
              </div>
              <div class="form-group">
                <label for="deadline-${
                  job.id
                }"><strong>Ngày hết hạn:</strong></label>
            ...
            job.deadline
              ...
              <div class="form-group">
                ...
                <select id="status-${
                  job.id
                }" name="status" style="width: 100%; margin-bottom: 10px;" required>
                  <option value="open" ${
                    normalizeStatus(job.status) === "open" ? "selected" : ""
                  }>Mở</option>
                  <option value="urgent" ${
                    normalizeStatus(job.status) === "urgent" ? "selected" : ""
                  }>Khẩn cấp</option>
                  <option value="closed" ${
                    normalizeStatus(job.status) === "closed" ? "selected" : ""
                  }>Đóng</option>
                </select>
              </div>
              <div style="margin-top: 10px;">
                <button type="button" data-job-id="${
                  job.id
                }" data-action="update" class="btn btn-primary">Cập nhật</button>
                <button type="button" data-job-id="${
                  job.id
                }" data-action="close-edit" class="btn btn-secondary">Đóng</button>
              </div>
            </form>
          `;
